...
Los endpoints distintos al login y la creación de usuarios requieren encabezado `Authorization: Bearer <token>`. La creación de usuarios permanece pública para permitir el registro inicial y asigna automáticamente el rol base `USER`. El endpoint `/api/v1/auth/me` requiere cualquier JWT válido. El módulo Rol requiere rol `ADMIN`. En Usuario, `ADMIN` puede administrar usuarios, mientras que `USER` y `OPERADOR` pueden consultar, actualizar y cambiar la contraseña únicamente de su propio usuario. En Estacionamiento, `ADMIN` consulta y administra todos los estacionamientos; `OWNER` crea estacionamientos asociados a su usuario autenticado y solo consulta, actualiza o elimina lógicamente sus propios estacionamientos; `OPERADOR` y `USER` conservan la consulta permitida actual. En Cajón, `ADMIN` puede consultar y administrar todos los cajones; `OWNER` puede consultar, crear, actualizar, cambiar estado y eliminar lógicamente cajones solo dentro de sus propios estacionamientos; `OPERADOR` puede consultar y cambiar estado; y `USER` puede consultar. En Catálogos, `ADMIN`, `OPERADOR` y `USER` pueden consultar los valores disponibles.
