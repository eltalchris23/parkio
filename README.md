...
Los endpoints distintos al login y la creación de usuarios requieren encabezado `Authorization: Bearer <token>`. La creación de usuarios permanece pública para permitir el registro inicial y asigna automáticamente el rol base `USER`. El endpoint `/api/v1/auth/me` requiere cualquier JWT válido. El módulo Rol requiere rol `ADMIN`. En Usuario, `ADMIN` puede administrar usuarios; cualquier usuario autenticado puede consultar, actualizar y cambiar la contraseña de su propio usuario; y `OWNER` puede consultar o actualizar operadores asignados a sus propios estacionamientos, además de asignar o retirar únicamente sus propios estacionamientos a usuarios que ya tengan rol `OPERADOR`. La asignación y retiro de roles sigue siendo exclusiva de `ADMIN`. En Estacionamiento, `ADMIN` consulta y administra todos los estacionamientos; `OWNER` crea estacionamientos asociados a su usuario autenticado y solo consulta, actualiza o elimina lógicamente sus propios estacionamientos; `OPERADOR` consulta únicamente estacionamientos asignados mediante `usuario_estacionamiento`; y `USER` conserva la consulta permitida actual. En Cajón, `ADMIN` puede consultar y administrar todos los cajones; `OWNER` puede consultar, crear, actualizar, cambiar estado y eliminar lógicamente cajones solo dentro de sus propios estacionamientos; `OPERADOR` puede consultar y cambiar estado únicamente en cajones de estacionamientos asignados; y `USER` puede consultar. En Catálogos, `ADMIN`, `OPERADOR` y `USER` pueden consultar los valores disponibles.
...
La autorización de Usuario usa `@PreAuthorize` y el helper `UsuarioSecurity` para comparar el `usuarioId` de la ruta contra el claim `usuarioId` del JWT y para validar el alcance de `OWNER` sobre operadores. `ADMIN` puede administrar usuarios; cualquier usuario autenticado puede consultar, actualizar y cambiar la contraseña de su propio usuario; `OWNER` puede consultar o actualizar operadores asignados a sus propios estacionamientos y puede asignar o retirar sus propios estacionamientos únicamente a usuarios que ya tengan rol `OPERADOR`. Las operaciones de asignación y retiro de roles son exclusivas de `ADMIN`.
...
Módulo funcional para crear y consultar reservas temporales de cajones.

Incluye:

- Entidad `Reserva`.
- `ReservaRequest` y `ReservaResponse`.
- `EstadoReserva`.
- `ReservaRepository`.
- `ReservaMapper`.
- `ReservaService`.
...
- `ReservaProperties`.
- `ReservaConfig`.
- `ReservaSchedulingConfig`.
- `ReservaScheduler`.
- Migración Flyway `V10__create_reserva.sql`.
- Pruebas unitarias de mapper, servicio y controlador.
- Prueba unitaria del scheduler.
- Prueba de integración `ReservaIntegrationTest`.

El enum `EstadoReserva` define los estados `CREADA`, `CANCELADA`, `EXPIRADA` y `USADA`.

El módulo expone endpoints para crear reservas, consultar las reservas del usuario autenticado, consultar por código público, consultar por identificador interno y cancelar reservas propias. La creación de una reserva valida usuario, estacionamiento, cajón disponible y ausencia de reserva vigente sobre el mismo cajón. Al crear una reserva, el cajón cambia a `RESERVADO`.

Para la creación de reservas se usa la propiedad `parkio.reserva.expiracion-minutos`, configurable mediante `PARKIO_RESERVA_EXPIRACION_MINUTOS`. El frontend no debe enviar la duración de la reserva; el backend calcula `fechaExpiracion` con la configuración vigente y guarda el tiempo aplicado en cada registro.

La cancelación manual permite que un usuario con rol `USER` cancele únicamente reservas propias que sigan en estado `CREADA` y vigentes. Al cancelar, la reserva cambia a `CANCELADA` y el cajón vuelve a `LIBRE` cuando no existe otra reserva vigente sobre el mismo cajón.
...
| `PARKIO_RESERVA_EXPIRACION_CHECK_MS` | Frecuencia en milisegundos para revisar y expirar reservas vencidas |
...
$env:PARKIO_RESERVA_EXPIRACION_CHECK_MS="60000"
```
