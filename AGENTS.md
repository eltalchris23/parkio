...
- El módulo Tarifa está integrado con el cierre de Ticket para calcular el cobro cuando se registra salida.
...
- El cierre de tickets mediante `registrarSalida` requiere tarifa activa del estacionamiento, calcula el cobro, guarda los parámetros aplicados, cambia tickets `ABIERTO` a `CERRADO`, registra `fechaSalida` y libera el cajón cambiándolo a `LIBRE`, aplicando el mismo alcance por rol de Ticket.
...
`EstadoTicket` define `ABIERTO` y `CERRADO`. Actualmente está implementada la creación de tickets abiertos mediante la conversión de una reserva vigente en entrada real y el cierre de tickets abiertos al registrar salida. El cierre calcula el cobro usando la tarifa activa del estacionamiento y guarda una copia histórica de los valores aplicados. No está implementada todavía la facturación, pagos ni emisión de comprobantes.

`TarifaEstacionamiento` representa la configuración de cobro de un estacionamiento mediante `precioPorHora`, `minutosTolerancia`, `cobrarFraccion` y `tarifaMinima`. La relación con `Estacionamiento` es uno a uno. Actualmente existe API REST para consultar, crear, actualizar y desactivar lógicamente tarifas activas. La tarifa activa se usa al cerrar tickets para calcular `montoTotal`, `minutosEstancia`, `precioPorHoraAplicado`, `minutosToleranciaAplicados`, `cobrarFraccionAplicado` y `tarifaMinimaAplicada`.
