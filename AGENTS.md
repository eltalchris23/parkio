...
- Los controladores principales Auth, Rol, Estacionamiento, Cajón, Usuario, Reserva, Ticket, Tarifa, Pago y Catálogos ya tienen anotaciones OpenAPI para Swagger UI.
...
- `UsuarioController` utiliza `@PreAuthorize` para permitir operaciones administrativas a `ADMIN`, operaciones propias a cualquier usuario autenticado y administración limitada de operadores por `OWNER` sobre sus propios estacionamientos. La asignación y retiro de roles sigue siendo exclusiva de `ADMIN`.
...
- `TicketController` utiliza `@PreAuthorize` para permitir consultas mediante `GET /api/v1/tickets` y `GET /api/v1/tickets/{ticketId}` a `ADMIN`, `OWNER`, `OPERADOR` y `USER` según alcance, y para permitir a `ADMIN`, `OWNER` y `OPERADOR` registrar entradas mediante `POST /api/v1/tickets/entrada` y salidas mediante `PATCH /api/v1/tickets/{ticketId}/salida`.
- `TarifaEstacionamientoController` utiliza `@PreAuthorize` para permitir a `ADMIN` administrar tarifas de cualquier estacionamiento y a `OWNER` administrar tarifas únicamente de estacionamientos propios.
- `PagoController` utiliza `@PreAuthorize` para permitir a `ADMIN`, `OWNER` y `OPERADOR` listar pagos mediante `GET /api/v1/pagos` y registrar pagos mediante `POST /api/v1/pagos` según alcance, y a `ADMIN`, `OWNER`, `OPERADOR` y `USER` consultar pagos por ticket mediante `GET /api/v1/pagos/ticket/{ticketId}` según alcance.
- `CatalogoController` utiliza `@PreAuthorize` para permitir consultas a `ADMIN`, `OPERADOR` y `USER`.
- `UsuarioSecurity` compara el `usuarioId` de la ruta con el claim `usuarioId` del JWT y valida el alcance de `OWNER` para administrar operadores asociados a sus estacionamientos.
...
- El listado de tickets `GET /api/v1/tickets` devuelve una respuesta estandarizada mediante `ApiResponse<PageResponse<TicketResponse>>` y acepta `page`, `size`, `sort`, `estado` y `estacionamientoId`. `ADMIN` ve todos los tickets activos, `OWNER` ve tickets de estacionamientos propios, `OPERADOR` ve tickets de estacionamientos asignados y `USER` ve sus propios tickets.
- Las operaciones no paginadas de Ticket para consultar por identificador, registrar entrada mediante `POST /api/v1/tickets/entrada` y registrar salida mediante `PATCH /api/v1/tickets/{ticketId}/salida` devuelven una respuesta estandarizada mediante `ApiResponse<TicketResponse>`.
- Las operaciones no paginadas de Tarifa para consultar por estacionamiento, crear y actualizar devuelven una respuesta estandarizada mediante `ApiResponse<TarifaEstacionamientoResponse>`.
- El listado de pagos `GET /api/v1/pagos` devuelve una respuesta estandarizada mediante `ApiResponse<PageResponse<PagoResponse>>` y acepta `page`, `size`, `sort`, `estacionamientoId`, `metodoPago`, `fechaInicio` y `fechaFin`. `ADMIN` ve todos los pagos activos, `OWNER` ve pagos de estacionamientos propios y `OPERADOR` ve pagos de estacionamientos asignados. `USER` no tiene acceso al listado general.
- Las operaciones no paginadas de Pago para registrar y consultar pagos por ticket devuelven una respuesta estandarizada mediante `ApiResponse<PagoResponse>`.
...
- `RolMapper`, `EstacionamientoMapper`, `CajonMapper`, `UsuarioMapper`, `ReservaMapper`, `TicketMapper`, `TarifaEstacionamientoMapper` y `PagoMapper` están implementados.
...
- Existe `TarifaEstacionamiento`, `TarifaEstacionamientoRequest`, `TarifaEstacionamientoResponse`, `TarifaEstacionamientoRepository`, `TarifaEstacionamientoMapper`, `TarifaEstacionamientoService`, `TarifaEstacionamientoServiceImpl`, `TarifaEstacionamientoController` y la migración Flyway `V12__create_tarifa_estacionamiento.sql`.
- El módulo Tarifa expone `GET /api/v1/tarifas/estacionamiento/{estacionamientoId}`, `POST /api/v1/tarifas`, `PUT /api/v1/tarifas/estacionamiento/{estacionamientoId}` y `DELETE /api/v1/tarifas/estacionamiento/{estacionamientoId}`.
- `TarifaEstacionamientoServiceImpl` permite operar tarifas a `ADMIN` de forma global y a `OWNER` solo sobre estacionamientos propios; `OPERADOR` y `USER` no administran tarifas. La eliminación es lógica mediante `activo=false`.
- El módulo Tarifa está integrado con el registro de salida de Ticket para calcular el cobro.
- Existe `Pago`, `EstadoPago`, `MetodoPago`, `PagoRequest`, `PagoResponse`, `PagoRepository`, `PagoMapper`, `PagoService`, `PagoServiceImpl`, `PagoController` y la migración Flyway `V15__create_pago.sql`.
- El módulo Pago expone `GET /api/v1/pagos`, `POST /api/v1/pagos` y `GET /api/v1/pagos/ticket/{ticketId}`. Listar y registrar pagos requiere `ADMIN`, `OWNER` u `OPERADOR` según alcance. Consultar pagos por ticket permite `ADMIN`, `OWNER`, `OPERADOR` y `USER` según alcance.
...
- La creación de tickets mediante `registrarEntrada` convierte reservas `CREADA` y vigentes en tickets `ABIERTO`, cambia la reserva a `USADA` y el cajón a `OCUPADO`. El usuario autenticado se obtiene desde el claim `usuarioId` del JWT. `ADMIN` puede operar cualquier estacionamiento, `OWNER` solo los propios y `OPERADOR` solo los estacionamientos asignados.
- El registro de salida mediante `registrarSalida` requiere tarifa activa del estacionamiento, calcula el cobro, guarda los parámetros aplicados, cambia tickets `ABIERTO` a `PENDIENTE_PAGO` y registra `fechaSalida`. El cajón permanece `OCUPADO` y no debe liberarse hasta que el pago quede registrado en el módulo Pago.
- El registro de pago mediante `registrarPago` valida tickets `PENDIENTE_PAGO`, calcula el cambio como `montoRecibido - montoTotal`, guarda el pago, cambia el ticket a `CERRADO` y libera el cajón a `LIBRE`.
...
- Existe `TicketIntegrationTest`, que levanta Spring Boot completo, usa PostgreSQL con perfil `test`, valida seguridad JWT/roles y prueba rechazo sin JWT, rechazo con rol `USER`, creación de ticket con `OPERADOR` asignado, creación con `ADMIN`, creación con `OWNER` sobre estacionamiento propio, consulta paginada de tickets, filtros por `estado` y `estacionamientoId`, consulta por identificador, cambio de reserva a `USADA`, cambio de cajón a `OCUPADO`, registro de salida con cambio a `PENDIENTE_PAGO`, bloqueo de doble ticket para la misma reserva y rechazo de operador no asignado al estacionamiento.
- Existe `TarifaEstacionamientoIntegrationTest`, que levanta Spring Boot completo, usa PostgreSQL con perfil `test`, valida seguridad JWT/roles y prueba rechazo sin JWT, rechazo con rol `USER`, creación, consulta, actualización, conflicto por duplicado, borrado lógico, alcance global de `ADMIN` y alcance limitado de `OWNER` sobre estacionamientos propios.
- Existe `PagoIntegrationTest`, que levanta Spring Boot completo, usa PostgreSQL con perfil `test`, valida rechazo sin JWT, registro de pago de ticket `PENDIENTE_PAGO`, cálculo de cambio, cambio de ticket a `CERRADO`, liberación del cajón a `LIBRE`, consulta de pago por ticket, listado paginado con filtros y bloqueo del listado general para `USER`.
...
- Usuario permite asignar y retirar roles y estacionamientos mediante `usuario_rol` y `usuario_estacionamiento`. `UsuarioResponse` representa estas relaciones mediante nombres de roles e identificadores de estacionamientos. La creación pública de usuarios asigna automáticamente el rol base `USER`. Creación, actualización general y cambio de contraseña utilizan DTOs y operaciones separadas. `OWNER` puede consultar o actualizar operadores asignados a sus estacionamientos propios, y puede asignar o retirar únicamente sus propios estacionamientos a usuarios que ya tengan rol `OPERADOR`; no puede asignar ni retirar roles.
...
`EstadoTicket` define `ABIERTO`, `PENDIENTE_PAGO` y `CERRADO`. Actualmente está implementada la creación de tickets abiertos mediante la conversión de una reserva vigente en entrada real, el registro de salida de tickets abiertos y el registro de pago de tickets pendientes. La salida calcula el cobro usando la tarifa activa del estacionamiento, guarda una copia histórica de los valores aplicados y deja el ticket en `PENDIENTE_PAGO`. El pago cambia el ticket a `CERRADO` y libera el cajón. No está implementada todavía la facturación fiscal ni emisión de comprobantes.

`Pago` representa la liquidación de un ticket mediante `montoTotal`, `montoRecibido`, `cambio`, `metodoPago`, `estado`, `fechaPago`, `ticket` y `operador`. `MetodoPago` define `EFECTIVO`, `TARJETA` y `TRANSFERENCIA`. `EstadoPago` define actualmente `REGISTRADO`. Un ticket solo puede tener un pago activo.

`TarifaEstacionamiento` representa la configuración de cobro de un estacionamiento mediante `precioPorHora`, `minutosTolerancia`, `cobrarFraccion` y `tarifaMinima`. La relación con `Estacionamiento` es uno a uno. Actualmente existe API REST para consultar, crear, actualizar y desactivar lógicamente tarifas activas. La tarifa activa se usa al registrar salida de tickets para calcular `montoTotal`, `minutosEstancia`, `precioPorHoraAplicado`, `minutosToleranciaAplicados`, `cobrarFraccionAplicado` y `tarifaMinimaAplicada`.
...
V13__add_cobro_to_ticket.sql
V14__add_pendiente_pago_to_ticket_estado.sql
V15__create_pago.sql
```
...
La tabla `ticket`, sus DTOs, repositorio, mapper, servicio y controlador ya existen. Las operaciones implementadas son `GET /api/v1/tickets`, `GET /api/v1/tickets/{ticketId}`, `POST /api/v1/tickets/entrada` y `PATCH /api/v1/tickets/{ticketId}/salida`. Las consultas permiten `ADMIN`, `OWNER`, `OPERADOR` y `USER` según alcance, y el listado acepta filtros opcionales `estado` y `estacionamientoId`. `ADMIN` tiene alcance global, `OWNER` solo estacionamientos propios, `OPERADOR` solo estacionamientos asignados y `USER` solo tickets propios. La entrada crea un ticket `ABIERTO`, cambia la reserva a `USADA` y el cajón a `OCUPADO`. La salida requiere tarifa activa, calcula el cobro mediante `TicketCobroCalculator`, guarda los valores aplicados en el ticket, cambia el ticket a `PENDIENTE_PAGO` y registra `fechaSalida`; el cajón permanece `OCUPADO`. La tarifa mínima se cobra desde el primer minuto.

La tabla `pago`, sus DTOs, enums, repositorio, mapper, servicio y controlador ya existen. Las operaciones implementadas son `GET /api/v1/pagos`, `POST /api/v1/pagos` y `GET /api/v1/pagos/ticket/{ticketId}`. El listado general es paginado y acepta filtros opcionales por estacionamiento, método de pago y rango de fechas. El registro de pago requiere que el ticket esté en `PENDIENTE_PAGO`, que tenga `montoTotal`, que no exista un pago activo previo y que `montoRecibido` sea mayor o igual al total. El backend calcula `cambio`, guarda el pago en estado `REGISTRADO`, cambia el ticket a `CERRADO` y cambia el cajón a `LIBRE`. No se debe asumir facturación fiscal, integración con pasarela o comprobantes hasta que se implemente explícitamente.

La tabla `tarifa_estacionamiento`, sus DTOs, repositorio, mapper, servicio y controlador ya existen. Los endpoints implementados son `GET /api/v1/tarifas/estacionamiento/{estacionamientoId}`, `POST /api/v1/tarifas`, `PUT /api/v1/tarifas/estacionamiento/{estacionamientoId}` y `DELETE /api/v1/tarifas/estacionamiento/{estacionamientoId}`. El módulo permite consultar, crear, actualizar y desactivar lógicamente tarifas activas con alcance `ADMIN` global y `OWNER` sobre estacionamientos propios. La tarifa activa se usa al registrar salida de Ticket para calcular el cobro.
...
- Existe autorización granular inicial por roles: `RolController` requiere `ADMIN`, `UsuarioController` protege operaciones con `ADMIN`, operaciones propias de cualquier usuario autenticado y alcance limitado de `OWNER` sobre operadores de sus estacionamientos, `EstacionamientoController` permite lectura a `ADMIN`/`OWNER`/`OPERADOR`/`USER`, escritura global a `ADMIN` y escritura propia a `OWNER`, `CajonController` permite lectura a `ADMIN`/`OWNER`/`OPERADOR`/`USER`, cambio de estado a `ADMIN`/`OWNER`/`OPERADOR`, escritura global a `ADMIN` y escritura propia a `OWNER`, `TicketController` permite consultas a `ADMIN`/`OWNER`/`OPERADOR`/`USER` según alcance y entrada/salida a `ADMIN`/`OWNER`/`OPERADOR` según alcance, `TarifaEstacionamientoController` permite administración a `ADMIN` global y a `OWNER` sobre estacionamientos propios, `PagoController` permite listado y registro a `ADMIN`/`OWNER`/`OPERADOR` según alcance y consulta por ticket a `ADMIN`/`OWNER`/`OPERADOR`/`USER` según alcance, y `CatalogoController` permite consulta a `ADMIN`/`OPERADOR`/`USER`.
...
- Contrato implementado de Auth, Rol, Estacionamiento, Cajón, Usuario, Reserva, Ticket, Tarifa, Pago y Catálogos.
