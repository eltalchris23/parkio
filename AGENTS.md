...
- `TicketController` utiliza `@PreAuthorize` para permitir a `ADMIN`, `OWNER` y `OPERADOR` registrar entradas mediante `POST /api/v1/tickets/entrada` y salidas mediante `PATCH /api/v1/tickets/{ticketId}/salida`.
...
- Las operaciones no paginadas de Ticket para registrar entrada mediante `POST /api/v1/tickets/entrada` y registrar salida mediante `PATCH /api/v1/tickets/{ticketId}/salida` devuelven una respuesta estandarizada mediante `ApiResponse<TicketResponse>`.
...
- El módulo Tarifa todavía no está integrado con el cálculo de cobro del cierre de ticket.
...
- El cierre de tickets mediante `registrarSalida` cambia tickets `ABIERTO` a `CERRADO`, registra `fechaSalida` y libera el cajón cambiándolo a `LIBRE`, aplicando el mismo alcance por rol de Ticket.
...
- Contrato implementado de Auth, Rol, Estacionamiento, Cajón, Usuario, Reserva, Ticket y Catálogos.
