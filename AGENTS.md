...
- Los controladores principales Auth, Rol, Estacionamiento, Cajón, Usuario, Reserva, Ticket, Tarifa y Catálogos ya tienen anotaciones OpenAPI para Swagger UI.
...
`EstadoTicket` define `ABIERTO`, `PENDIENTE_PAGO` y `CERRADO`. Actualmente está implementada la creación de tickets abiertos mediante la conversión de una reserva vigente en entrada real y el registro de salida de tickets abiertos. La salida calcula el cobro usando la tarifa activa del estacionamiento, guarda una copia histórica de los valores aplicados y deja el ticket en `PENDIENTE_PAGO`. `CERRADO` queda reservado para cuando el pago sea registrado. No está implementada todavía la facturación, pagos ni emisión de comprobantes.
...
- Contrato implementado de Auth, Rol, Estacionamiento, Cajón, Usuario, Reserva, Ticket, Tarifa y Catálogos.
